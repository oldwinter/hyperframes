...
drawElement 并行捕获开始向 5% 的安装灰度开放，用于加速长时间渲染；一旦发生
fallback，该安装会持续关闭并行路径。可用 `HF_DE_PARALLEL_ROUTER=false` 退出，
或设为 `true` 提前启用。本版本同时修复 Studio 中三处会触发崩溃边界的问题。
...
被其他 composition 挂载的组件现在会在 live preview 中保留自己的 style，修复预览
无样式但最终视频正常的问题。软件渲染下的 screenshot capture 不再叠加旧 frame；
Studio 在窗口空间很小时也能保持 preview 存活，并补齐多项 preview/render 一致性修复。
...
发布上传遇到短暂网络故障时会自动重试；无法恢复时会明确指出失败步骤。长时间 timeout
不会被盲目重试，signed upload secret 也不会出现在错误信息中。
...
失败的 render 现在会记录 parallel drawElement router 是否处于 active 状态，避免把
“从未使用 router”和“使用后 crash/hang”混为同一种失败。该版本只补充诊断状态，
不会改变实际 render 路径。
...
Studio 调整 animated element 尺寸后，会把元素固定在实际 drop point。保存的 GSAP
scale 和 position 与 gesture 期间显示的 frame 保持一致，避免松手后发生跳动；legacy、
旋转、uniform 和分轴 resize path 均覆盖这一行为。
...
CLI 与 Studio 的 canary rollout 现在会记录每个安装进入或离开 cohort 的原因，并说明
anonymous id 是否会在下次运行继续存在。用户可见改动还包括 rooted timeline media
预览修复、可取消的 thumbnail generation，以及一轮大规模 docs example 更新。
...
本版本修复嵌套项目布局中 rooted 时间线媒体在预览时走错资源路由、导致 clip 缺失的
问题；缩略图生成可在快速编辑下取消。文档与 catalog 改用真实影片、可访问播放、
更清晰的工作流地图和经源校验的 SDK 指引重建。
...
CLI 新增基于 RFC 8628 的设备授权登录，适用于有人值守的 SSH 与远程终端会话：
无需复制 API key 或暴露 loopback 回调；流程 fail-closed，minted token 在原子
持久化前先校验，原有浏览器登录保持不变。
...
Studio 不再静默覆盖外部文件编辑，改为保留并恢复冲突；逻辑时间线导航让键盘
操作更可预测。Engine 与 producer 同时加固远程下载、ffprobe 调用、telemetry
路径脱敏、GPU 校验和跨帧率 BeginFrame 时序。
...
落地基于百分比的 canary 灰度发布：可在本地离线决定，把变更推给稳定的安装子集，
并以两个惰性校准 canary 先验证机制。同时新增三个主题族共 29 个 registry 项、
更快的虚拟化 Studio 时间线，以及子合成 sibling 资源路径的全链路解析。
...
阻止短视频 composition 解码整个长视频源、因 HDR raw-frame scratch 与文件系统
page cache 耗尽 producer 内存：抽取现在只跟随可播放时间线，并发渲染共享有界
scratch 容量，并在所有退出路径清理资源。Producer API 新增
`outputDynamicRange: auto | hdr | sdr`，过渡期仍接受旧字段 `hdrMode`。
...
生成的 waveform、thumbnail 与 transcode 缓存不再触发 Studio 项目重载，也不会
作为项目文件出现；真实源文件编辑仍正常失效预览。
...
本版本同时收紧媒体探测的输入、大小、编解码和 PNG 解析边界，并新增色彩分级 Prompt
Guide 章节。Color Grading guide 现在说明如何动画化可 tween 的 grade 属性，以及
如何通过独立媒体层、matte 和 `clip-path` 处理局部画面；新增内容保留上游命令、
链接和技术契约不变。
