...
v0.7.88 加固了媒体探测边界：限制 `ffprobe` 输入和输出、使 AAC 时长细化可取消且
仅允许 AAC-LC、严格处理帧率与 PNG 检查，并兼容 Node 22.0 和 22.1 的 PNG CRC。

文档新增 Color grading and film effects Prompt Guide 章节，配套十组 A/B render，
覆盖色调处理、色相选择、印刷和模拟媒介效果，以及“源素材比 payload 更重要”的
提示词原则。Color Grading guide 新增 grade 动画能力说明，以及通过分层、matte 和
`clip-path` 将 grading 限制在部分画面的做法。
