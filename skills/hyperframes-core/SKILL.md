...
这是 `hyperframes-core` 的中文 runtime 入口。当前契约要点：standalone root 不再要求 `data-start="0"`；sub-composition 使用 `<template>` 时，把 `<style>` / `<script>` 放进 template；timeline 可以在 `document.fonts.ready` 等异步构建完成后再注册，但必须只注册完成的 timeline。`data-track-index` 是 Studio 展示 lane，不是 timing 约束；video 不要嵌在另一个带 `data-start` 的普通元素中，audio 必须有 `id`。保持媒体 id 唯一，并注意 `<source>` 子节点媒体不会获得自动 `data-hf-render-id`。
