...
这是 `hyperframes` 的中文 runtime 入口。涉及多个轨道共享一套效果链、fader 或 automation clock 时，路由到 `/hyperframes-audio` 并使用 `<hf-audio-group>` submix bus；voiceover carve 仍须引用纯 voiceover group，不能把 bed 或 carve bus 混入该分组。
